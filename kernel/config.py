...
def load_config(path: str | Path | None=None) -> Config:
    ...
    g = raw.get('general', {})
    general = GeneralConfig(timezone=g.get('timezone', 'Asia/Shanghai'), default_provider=g.get('default_provider', 'anthropic'), default_workspace=g.get('default_workspace', '.'), context_rounds=g.get('context_rounds', 50), memory_recall_k=g.get('memory_recall_k', 5), data_dir=g.get('data_dir', 'data'))
    providers: dict[str, ProviderConfig] = {}
    for name, praw in raw.get('providers', {}).items():
        ...
        try:
            ...
            headers = None
        providers[name] = ProviderConfig(name=name, type=ptype, api_key=api_key, default_model=praw.get('default_model', ''), models=praw.get('models', []), api_base=praw.get('api_base'), max_tokens=max_tokens, headers=headers)
    ...
    if 'titles' in raw:
        ...
        if t_api_key and t_api_key != 'sk-...':
            try:
                ...
                t_headers = None
            titles = TitlesConfig(type=t.get('type', 'openai_compat'), api_base=t.get('api_base', ''), api_key=t_api_key, model=t.get('model', ''), max_tokens=t.get('max_tokens', 100), headers=t_headers)
        else:
    ...
    if 'stt' in raw:
        ...
        if s_api_key and s_api_key != 'sk-...':
            try:
                ...
                s_headers = None
            stt = STTConfig(api_base=s.get('api_base', 'https://api.openai.com/v1'), api_key=s_api_key, model=s.get('model', 'whisper-1'), headers=s_headers)
        else:
    ...
    for s in raw.get('mcp', {}).get('servers', []):
        try:
            ...
            continue
        mcp_servers.append(MCPServerConfig(name=s.get('name', 'unnamed'), type=s.get('type', 'stdio'), url=s.get('url'), command=s.get('command'), args=s.get('args'), headers=hdrs))
    return Config(telegram=telegram, general=general, providers=providers, titles=titles, stt=stt, tts=tts, cli=cli, mcp_servers=mcp_servers, config_dir=config_dir)
