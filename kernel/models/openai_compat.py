"""OpenAI-compatible provider — ``/v1/chat/completions`` with streaming + tool use + vision."""
...
# ---------------------------------------------------------------------------
# Format conversion helpers (internal ↔ OpenAI)
# ---------------------------------------------------------------------------


def _to_openai_content(blocks: list[ContentBlock] | str) -> str | list[dict[str, Any]]:
    """Convert internal content blocks to OpenAI message content."""
    if isinstance(blocks, str):
        return blocks
    # If only text blocks, return plain string for simplicity
            ...
            if finish:
                ...
                for _idx in sorted(tool_calls_acc):
                    ...
                    yield StreamChunk(
                        tool_use_id=acc["id"],
                        tool_name=acc["name"],
                        tool_input_json="".join(acc["arguments"]),
                    )
