"""Anthropic Claude provider — Messages API with streaming + tool use + vision."""
...
# ---------------------------------------------------------------------------
# Format conversion helpers (internal ↔ Anthropic)
# ---------------------------------------------------------------------------


def _to_anthropic_content(blocks: list[ContentBlock] | str) -> list[dict[str, Any]] | str:
    """Convert internal content blocks to Anthropic API format."""
...
def _to_anthropic_messages(messages: list[Message]) -> list[dict[str, Any]]:
    ...
    Anthropic expects alternating user/assistant messages.  ``tool_result``
    role messages are sent as ``user`` role with tool_result content blocks.
    """
    out: list[dict[str, Any]] = []
    for msg in messages:
...
def _from_anthropic_content(blocks: list[dict[str, Any]]) -> list[ContentBlock]:
    """Convert Anthropic response content blocks to internal format."""
...
class ClaudeLLM(LLM):
    ...
    async def chat(
        ...
        *,
        ...
        return LLMResponse(
            content=_from_anthropic_content([b.model_dump() for b in resp.content]),
