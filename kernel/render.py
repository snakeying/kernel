...
class TelegramHTMLRenderer(BaseRenderer):
    ...
    def list_item(self, text: str, **attrs: Any) -> str:
        text = text.strip()
        return f'• {text}\n'
_md = mistune.create_markdown(renderer=TelegramHTMLRenderer(), plugins=['strikethrough'])

def md_to_tg_html(text: str) -> str:
    return _md(text)
TG_MAX_LEN = 4096
_OPEN_TAG_RE = re.compile('<(b|i|u|s|code|pre|a|tg-spoiler)(?:\\s[^>]*)?>', re.IGNORECASE)
_CLOSE_TAG_RE = re.compile('</(b|i|u|s|code|pre|a|tg-spoiler)>', re.IGNORECASE)
