...
from kernel.models.base import ContentBlock, ImageContent, LLM, Message, Role, StreamChunk, TextContent, ToolDef, ToolResultContent, ToolUseContent
    ...
    def _tool_safe_history(self, messages: list[Message]) -> list[Message]:
        ...
        for msg in cleaned:
            if msg.role != Role.TOOL_RESULT or not isinstance(msg.content, list):
                final.append(msg)
                continue
            new_blocks = [b for b in msg.content if isinstance(b, ToolResultContent) and b.tool_use_id in tool_use_ids]
            if new_blocks:
